...
export function softReset(tempBulk, forcedADReset = false, forcedAMReset = false, enteringAntimatterChallenge = false) {
  ...
  // Check for Companions Tab Unlock
  if (!player.companions.records.hasUnlockedTab && player.dimensionBoosts >= 1) {
    player.companions.records.hasUnlockedTab = true;
    GameUI.notify.success("Companions Tab Unlocked!");
  }
