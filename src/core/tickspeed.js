...
export function effectiveBaseGalaxies() {
  ...
  return Math.max(player.galaxies + GalaxyGenerator.galaxies + replicantiGalaxies + freeGalaxies + Companions.totalEffect("free_galaxy").toNumber(), 0);
}
...
export function getTickSpeedMultiplier() {
  ...
  let mult = perGalaxy.pow(galaxies - 2).times(baseMultiplier);
  mult = mult.times(Companions.totalEffect("tickspeed"));
  return mult;
}
...
export const Tickspeed = {
  ...
  get baseValue() {
    return DC.E3.timesEffectsOf(
      ...
      .dividedBy(Companions.totalEffect("tickspeed"));
  },
